...
function load()
 ...
 g_game.enableFeature(GameSpritesU32) local datPath, sprPath
  if filename then
    datPath = resolvepath('/Imagens/' .. filename)
    sprPath = resolvepath('/Imagens/' .. filename)
  else
    datPath = resolvepath('/Imagens/' .. version .. '/Tibia')
    sprPath = resolvepath('/Imagens/' .. version .. '/Tibia')
  end
